...
class SSPSpace(Space):
    def __init__(self, basis, bounds, shape=None, dtype=complex, alg = HrrAlgebra()):
    ...
    def sample(self):
        ...
        pt = unif_dist.sample(1,self.nbasis)*(self.bounds[:,1] - self.bounds[:,0]) + self.bounds[:,0]
        sample = spa.SemanticPointer(data=np.fft.ifft(np.fft.fft(self.basis[0], axis=0)**pt[0,0] , axis=0).real)
        for i in np.arange(1,self.nbasis):
            nextS = spa.SemanticPointer(data=np.fft.ifft(np.fft.fft(self.basis[i], axis=0)**pt[0,i] , axis=0).real)
            sample = sample * nextS
        return sample.v
    
    def samples(self,n):
        unif_dist = UniformHypersphere()
        pt = unif_dist.sample(n,self.nbasis)*(self.bounds[:,1] - self.bounds[:,0]) + self.bounds[:,0]
        samples= np.zeros((n,self.dim))
        for j in np.arange(n):
            sample = spa.SemanticPointer(data=np.fft.ifft(np.fft.fft(self.basis[0], axis=0)**pt[j,0] , axis=0).real)
            for i in np.arange(1,self.nbasis):
                nextS = spa.SemanticPointer(data=np.fft.ifft(np.fft.fft(self.basis[i], axis=0)**pt[j,i] , axis=0).real)
                sample= sample * nextS
            samples[j,:] = sample.v
        return samples
    ...
    def contains(self, x):
        ...
        return (type(x) == SemanticPointer) & (len(x) == self.dim)
